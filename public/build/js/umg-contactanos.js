(() => {
  const form = document.querySelector('.umg-contactanos [data-contact-form]');
  if (!form) return;
  ...
  const statusEl = document.querySelector('.umg-contactanos [data-contact-status]');
  ...
  const setStatus = (message, isError = false) => {
    ...
    statusEl.classList.toggle('umg-contactanos__status--error', isError);
  };
  ...
  if (radios.length && channelsReady) {
    radios.forEach((radio) => {
      radio.addEventListener('change', () => {
        if (radio.checked) activateChannel(radio.value);
      });
    });
  ...
  const root = document.querySelector('.umg-contactanos');
  if (root && !window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
    root.classList.add('umg-contactanos--ready');
  }
  ...
  form.addEventListener('submit', async (event) => {
    ...
    formData.set('subject', `Contáctanos UMG Guastatoya — ${interesLabel}`);
    ...
    const originalText = submitLabel?.textContent || 'Enviar mensaje';
    ...
    try {
      ...
      if (response.ok && data.success !== false) {
        setStatus('Recibimos tu mensaje. Te contactaremos pronto.');
        form.reset();
